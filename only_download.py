...
from web_ops import get_authors_proxy,get_random_ua,decode_cite_text
...
def setWebDriver(save_path):
    ...
    proxyAddr = "156.231.80.205:31186"
    authKey = "7F2B4D16"
    password = "E43B2B32722A"
        # 账密模式
...
if __name__ == '__main__':
    ...
    datass = []
    with open(f'./{paper_namex}_full_report.csv','r',encoding='utf-8',errors='ignore') as f:
            for line in f:
    ...
    if is_authors:
        ...
        with open(f'./{paper_namex}_authors_infos.csv','r',encoding='utf-8',errors='ignore') as f:
            lines = f.readlines()
        for i in tqdm(range(len(lines)),desc='Getting authors...'):
            line = lines[i]
            ...
            # authors = get_authors_proxy(paper_id,page,x,is_debug)
            # if is_debug:
            ...
            # #     elif authors[0] == 'bot_403':
            # #         print('Get Unknow forbidden , Try reset for a few minutes')
            # #         print(f'To avoid bot check, will wait for {wait_count} minutes')
            # #         time.sleep(wait_count*60)
            # #         wait_count = wait_count*2
                
            # with open(f'./{paper_namex}_authors.csv','a',encoding='utf-8',errors='ignore') as f:
            #     f.write(f'{line[0]},{';'.join(authors)}\n')
            # ath_get_count += 1
